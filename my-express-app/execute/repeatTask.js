...
async function insertTaskFromRepeat(task) {
  ...
  await executeQuery(updateQuery, [nextRun.format('YYYY-MM-DD HH:mm:ss'), task.id]);
}
...
async function processRepeatTasks() {
  ...
  if (tasks && tasks.length > 0) {
    for (const task of tasks) {
      await insertTaskFromRepeat(task);
    }
  }
}
...
module.exports = processRepeatTasks;